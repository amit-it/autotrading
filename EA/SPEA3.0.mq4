...
void OnTick() {
    int dbConnectId = 0;
    dbConnect = init_MySQL(dbConnectId, host, user, pass, dbName, port, socket, client);
    ...
    //+-------------------------------------------------------------------
    string ct = TimeToStr(TimeCurrent(),TIME_DATE);
    StringReplace(ct,".","");
    string row[][8];   // important: second dimension size must be equal to the number of columns
    string rowQuery = StringConcatenate("SELECT `pair`, `Date`, `buy/sell`, `weight`, `Exit`, `stoploss`, `Max`, `Min` FROM trade.strats2 where `pair`='",Symbol(),"' and Date='",ct,"'")  ;  
    Print(rowQuery);
    int row_result = MySQL_FetchArray(dbConnectId, rowQuery, row);
    string line = "One row query: ";
