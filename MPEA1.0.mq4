...
void OnTick()
  {
   ...
   //Print(WasNoTradeWithinLAST(iTime( _Symbol, PERIOD_D1, 1 )));
   ...
   if(Opened() <= 0 || !IsTradeExistInHistory())
     {
      //--- no opened orders identified
       if(AccountFreeMargin()<(16*oLots))
        ...
        //else {
      //--- BUY
      //Print("modelPredictedDetails "+modelPredictedDetails[1]);
      ...
      //Print("Ask:",Ask," modelPredictedDetails:",modelPredictedDetails," Bid:",Bid,"Stoploss:",stoploss,"TakeProfit",takeprofit);
      if(modelPredictedDetails[1]==1)
      {
        //Print("Entering to Buy");
         ...
         if(ticket>0)
           {
            //if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES))
              ...
              // Print("BUY order opened and closed : ",OrderOpenPrice());
           }
         else
      ...
      if(modelPredictedDetails[1]==-1)
        {
        //Print("Entering to Sell");
         ticket=OrderSend(Symbol(),OP_SELL,LotsOptimized(),Bid,3,stoploss,takeprofit,"",MAGICMA,0,Red);
         if(ticket>0)
           {
            //if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES))
               //OrderClose( OrderTicket(), OrderLots(), MarketInfo(OrderSymbol(), MODE_ASK), 5, CLR_NONE );
              // Print("SELL order opened and closed : ",OrderOpenPrice());
           }
         else
   ...
   if(modelPredictedDetails[1]==-1)
   {
      double existPercentValue = (MarketInfo(Symbol(), MODE_ASK) - PriceWhenOrderOpendForCurrentPair()) / PriceWhenOrderOpendForCurrentPair();
      Print("CurrentPrice:"+MarketInfo(Symbol(), MODE_ASK)+"InitialPrice:"+PriceWhenOrderOpendForCurrentPair()+",Sell ExistPV:"+existPercentValue+" ModelExitPV"+modelPredictedDetails[3]);
               
      if(existPercentValue >  modelPredictedDetails[3])      
               {
               //Print(MarketInfo(Symbol(), MODE_ASK)+"Closing Sell orders more than "+modelPredictedDetails[3] +" for "+Symbol());
               /*
                  if(AllSymbols)
                     {
                        if(PendingOrders)
